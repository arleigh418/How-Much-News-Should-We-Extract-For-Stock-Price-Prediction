...
folder = ['article_stop_word'] #article set
        ...
        for model in count_model:
            for way in count_way:
                ...
                #Read Excel - stock price and word emb file
                    ...
                    word_emb_data = pd.read_excel(f'../{fo}/All_File/{com_list_num[com]}TW_intro/{way}/{way}{per}%_{model}_stock_{com_list[com]}.xlsx')
                    ...
                    final_merge_file.to_excel(f'../{fo}/{com_list_num[com]}Final_Result_intro/add_method_file/{way}/{way}{per}_{model}_final{com_list[com]}.xlsx',index=False)
