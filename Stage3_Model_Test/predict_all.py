...
count_com = ['6180','2886','2317','2324','2412'] #'2317','2324','2412',
count_name = ['橘子','兆豐金','鴻海','仁寶','中華電信'] #鴻海','仁寶','中華電信',
...
for word in count_word:
    for com in range(len(count_name)):
                ...
                for way in count_way:
                    for per in count_per:
                        try:
                            ...
                            data= pd.read_excel(f"../{word}/{count_com[com]}Final_Result/{method}_method_file/{way}/{way}{per}_{model_use}_final{count_name[com]}.xlsx", debug=False)
                            ...
                            print(f"../{word}/{count_com[com]}Final_Result/model/{method}_method_model/{way}{per}{model_use}model_{method}.pkl")
                            lstm = LSTM(num_classes, input_size, hidden_size, num_layers)
                                # y_train = model.train()
                            lstm.load_state_dict(torch.load(f"../{word}/{count_com[com]}Final_Result/model/{method}_method_model/{way}{per}{model_use}model_{method}.pkl"))
                            

                            test_predict = lstm(testX)
                            result = count_values(testY,test_predict)
                            
                            record_text.append(f"../{word}/{count_com[com]}Final_Result/model/{method}_method_model/{way}{per}{model_use}model_{method}.pkl")
        ...
        storage.to_excel(f'{word}_{count_com[com]}Record.xlsx')
                        
        fig3 = plt.figure()
        plt.title(u'{0}.TW - Model Predict Result({1})'.format(count_com[com],word))
        ...
        plt.legend(loc='best')
        plt.savefig(f"../test_result/{count_com[com]}TW/{count_com[com]}lstm_{word}.png")
        plt.close(fig3)
